...
def add_period(
    start_date: Date,
    term: Period,
    business_day_convention: Optional[BusinessDayConvention] = BusinessDayConvention(
        "F"
    ),
    ...
    return Date(
        this_cal.advance(start_date, term, business_day_convention.value, end_of_month)
    )
...
def move_to_business_day(
    ...
    return Date(
        holiday_convention.value.adjust(input_date, business_day_convention.value)
    )
...
def accrued(
    ...
    adjusted_end_dt = move_to_business_day(
        end_date, business_day_convention, holiday_convention
    )
...
def make_schedule(
    ...
    this_rule = (
        ...
        if rule.upper() == "BACKWARD"
        else ql.DateGeneration.Forward
    )
    ...
    for s, e in zip(start_dates, end_dates):
        ...
        accs.append(
            accrued(s, e, accrual_basis, business_day_convention, holiday_convention)
        )

    # set up container
    df = pd.DataFrame(
        columns=["StartDate", "EndDate", "FixingDate", "PaymentDate", "Accrued"]
    )
